...
def verify(username = "", password = "", session_id = "",id=0):
	ff = open('recor%d.txt'%(id),'a')
...
def go(start=0, end=0, l=[], id=0, session_id=""):
	print("#####",id)
	counter = 0
	number = len(l)
	for o in l:
		...
		ret = verify(username=o['username'],password=o['password'],session_id=session_id, id=id)
		if ret == "445":
...
#	verify()
	...
	for i in range(10):
		...
		try:
			...
			thread.start_new_thread(go,(start, end, l, i, session_id))
		except Exception, e:
			print("thread %d failed to start. %s"%(i,e))
